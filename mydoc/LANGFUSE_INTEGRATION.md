...
| 項目                 | 説明                                    |
| -------------------- | --------------------------------------- |
| **Trace**            | リクエスト全体（HTTP メソッド + path）  |
| **Span**             | `wrapHandler()` を通る handler 実行     |
| **ステータスコード** | handler が返した HTTP レスポンスコード  |
| **遅延**             | trace/span の開始から終了までの処理時間 |
| **エラー情報**       | handler 例外時のエラー message          |

現時点の実装は API リクエスト単位の tracing が中心です。LLM の prompt、completion、model name、token usage などの generation 情報はまだ自動記録していません。必要な場合は Bedrock 呼び出し箇所に Langfuse generation を追加します。
