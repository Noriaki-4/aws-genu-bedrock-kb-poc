...
## 13. 大容量PDFの分割取り込み（2026-07-21 実装）

276ページPDFがparserのスロットリングで失敗した問題（4.14）への対応として、事前分割と
逐次投入のCLIを追加した。メタデータは
[GENU_RAG_METADATA_DEFINITION.md](./GENU_RAG_METADATA_DEFINITION.md)に準拠する。

### 13.1 機械的な分割で意味のつながりが失われない理由

- Foundation Model parserはページを画像として個別に処理するため、分割してもparserの
  入力単位は変わらない。
- 高度解析時のチャンカーはページ境界をまたがない。AWSのchunkingドキュメントに
  "The chunker respects logical document boundaries (such as pages or sections) and does not
  merge content across these boundaries" と明記されている。チャンクは元々ページ内で閉じており、
  ページ境界で分割する限り新たな意味的分断は発生しない。
- 文書としてのまとまりは、全パートで`document_id`、`document_title`、`original_file_name`を
  同一にし、`document_title`だけをEmbeddingに含めることで維持する。
- S3 Vectorsでは階層チャンキングが非推奨のため、固定長1500トークン・20%オーバーラップを維持する。

### 13.2 追加したファイル

| ファイル                                                  | 役割                                                    |
| --------------------------------------------------------- | ------------------------------------------------------- |
| `packages/cdk/tools/rag-docs/core/manifest.ts`            | 文書マニフェストのzodスキーマと文書メタデータの組み立て |
| `packages/cdk/tools/rag-docs/core/splitPlan.ts`           | ページレンジの算出（固定ページ数または手動レンジ）      |
| `packages/cdk/tools/rag-docs/core/sidecar.ts`             | `metadataAttributes`形式のsidecar生成                   |
| `packages/cdk/tools/rag-docs/core/validate.ts`            | メタデータ定義8章とS3 Vectors制約の検証                 |
| `packages/cdk/tools/rag-docs/core/partArtifacts.ts`       | 分割ファイル命名と`parts.json`                          |
| `packages/cdk/tools/rag-docs/core/ingestionPlan.ts`       | 投入対象の選択と重複オブジェクト検出                    |
| `packages/cdk/tools/rag-docs/cli/split.ts`                | 分割CLI（pdf-lib）                                      |
| `packages/cdk/tools/rag-docs/cli/ingest.ts`               | 逐次アップロードとingestion job実行                     |
| `packages/cdk/rag-docs/manifests/EXAMPLE-MANUAL-001.yaml` | マニフェストの雛形                                      |

`core/`はNode.jsとAWS SDKに依存しない純粋関数で構成し、将来のLambda化に備えている。

### 13.3 運用手順

1. マニフェストを作成する。`metadata_schema_version`と`content_hash`はCLIが埋める。

```bash
cp packages/cdk/rag-docs/manifests/EXAMPLE-MANUAL-001.yaml \
   packages/cdk/rag-docs/manifests/<DOCUMENT_ID>.yaml
```

2. 分割する。既定は50ページ。`split.ranges`で章境界を手動指定できる。

```bash
cd packages/cdk
NODE_OPTIONS=--max-old-space-size=4096 npx ts-node tools/rag-docs/cli/split.ts \
  --manifest rag-docs/manifests/<DOCUMENT_ID>.yaml \
  --input <原本PDFのパス> \
  --out-dir /tmp/rag-parts/<DOCUMENT_ID> \
  --pages-per-part 50
```
...
```bash
AWS_PROFILE=rag-poc-admin aws cloudformation describe-stacks \
  --stack-name RagKnowledgeBaseStackdev \
  --query 'Stacks[0].Outputs' --region ap-northeast-1
```

4. 1パートずつ逐次同期する。失敗時は`--start-from`で失敗パートから再開する。

```bash
AWS_PROFILE=rag-poc-admin npx ts-node tools/rag-docs/cli/ingest.ts \
  --parts /tmp/rag-parts/<DOCUMENT_ID>/parts.json \
  --bucket <DataSourceBucketName> \
  --prefix docs/<DOCUMENT_ID>/ \
  --knowledge-base-id <KnowledgeBaseId> \
  --data-source-id <DataSourceId> \
  --region ap-northeast-1
```

`StartIngestionJob`は0.1 rps・データソースあたり同時1ジョブのため、CLIはジョブ起動前に15秒待ち、
30秒間隔でポーリングする。増分同期のため、取り込み済みのパートは再解析されない。
スロットリングが続く場合は`--pages-per-part 25`で再分割する。

### 13.4 出典ページの原本復元

分割PDFではBedrockが返すページ番号がパート内のページになるため、`original_page_start`から
原本ページを復元する（メタデータ定義6.3）。

```text
パート内ページ = toOneBasedPageNumber(x-amz-bedrock-kb-document-page-number)
原本ページ     = original_page_start + パート内ページ - 1
```

出典ラベルには原本ページ、リンクの`#page=`にはパート内ページを使用する。リンク先はS3上の
分割PDFなので、クリックすると実際に該当箇所が開く。

```markdown
[^0]: [設備障害対応マニュアル(73 page)](.../manual_p0051-0100.pdf#page=23)
```

表示名は`document_title` → `original_file_name`（拡張子除去）→ 既存の`title` → URLのファイル名の
順で解決する。これにより同一文書の複数パートが1つの出典名にまとまる。
未分割文書では`original_page_start`が存在しないため、従来どおりの表示になる。

### 13.5 S3 Vectorsのメタデータ制約

| 項目                            | 上限   | 対応                                                       |
| ------------------------------- | ------ | ---------------------------------------------------------- |
| フィルタ可能メタデータ/ベクトル | 2 KB   | `validate.ts`が推定バイト数を検証（システム用に512B確保）  |
| メタデータキー数/ベクトル       | 50     | 現行のsidecarは最大19キー                                  |
| 非フィルタキー数/インデックス   | 10     | 現在2キー使用。超過時は表示専用項目を退避できる            |
| sidecarファイルサイズ           | 1024 B | `validate.ts`が検証。ドキュメント記載の10 KBは実機と不一致 |

2 KBを超えた場合は`rag-knowledge-base-stack.ts`の`nonFilterableMetadataKeys`へ
`document_title`、`original_file_name`、`source_url`、`content_hash`を追加する。ただし
`CfnIndex`の置換（全ベクトル再取り込み）が発生するため、実測で超過するまでは変更しない。

### 13.6 検証結果（2026-07-21）

| 項目                        | 結果                                                    |
| --------------------------- | ------------------------------------------------------- |
| ユニットテスト              | 17スイート・166テスト成功（うち新規82テスト）           |
| lint / 型チェック           | 警告0 / `tsc --noEmit`成功                              |
| 分割CLI実機（276ページPDF） | 6パート＋6 sidecar＋`parts.json`を生成                  |
| sidecar形式                 | 混在形式・minifyで693バイト（上限1024バイト）           |
| メタデータ形式の実機検証    | 混在形式が受理され、`includeForEmbedding: true`も保持   |
| Retrieveでのメタデータ取得  | `original_page_start=117`ほか全属性の返却を確認         |
| 141ページPDFの投入          | 8ページ/パート・18パートを逐次投入し全パート成功・失敗0 |

### 13.7 出典リンクと表示の改善（2026-07-23）

画面確認で、分割PDFへのリンクがビューア表示とずれる問題と、文書タイプ差の問題が判明したため、
出典生成を次のように変更した。詳細と設計判断は
[GENU_KB_PDF_ADVANCED_PARSING_TROUBLESHOOTING.md](./GENU_KB_PDF_ADVANCED_PARSING_TROUBLESHOOTING.md)
の課題16〜18（4.18〜4.20）にまとめている。

| 変更             | 内容                                                                                 |
| ---------------- | ------------------------------------------------------------------------------------ |
| 出典リンク先     | 分割PDF → 統合PDF（`originals/`）。`#page=`は原本物理ページ                          |
| ページ表記       | 物理ページで統一。PageLabels（論理ページ）変換は行わない                             |
| ページ番号の対象 | Bedrockがページ番号を返すPDFのみ。DOCX等は出さない                                   |
| 抜粋併記         | `content.text`の冒頭を脚注へ併記。ページ非依存の箇所特定手段                         |
| 脚注レイアウト   | 出典タイトルと抜粋を`remark-breaks`の改行で別行にする                                |
| 実装範囲         | すべてLambda（`bedrockKbCitation`/`bedrockKbApi`/`bedrockAgentApi`）。フロント無改修 |
| 統合PDFの配置    | split CLIが出力、ingest CLIが`originals/`へアップロード（解析対象外）                |

検証結果は次のとおり。

- lint / 型チェック / 全テスト成功（citation 12テスト＋rag-docs等を含む）
- `GenerativeAiUseCasesStackdev`をデプロイ（IAM変更なし・Lambdaアセットのみ）
- RAGチャットで出典ラベルが原本ページ（例：119 / 137 / 136 page）で表示され、リンクが統合PDFの
  該当ページを開き、抜粋が別行に表示されることを画面で確認
- `originals/`の配信アクセス制御は`docs/`と同一（バケット全体への`s3:GetObject`付与）であることを確認

### 13.8 残作業

1. アクセス制御フィルター（メタデータ定義7章）を次フェーズで実装する。その際、`STRING_LIST`が
   二重引用符付きで返る挙動が`in`条件へ影響しないかを確認する。
2. 大容量PDFの本番投入では、Service Quotasで
   `Cross-region model inference requests per minute`の引き上げを申請する。
3. PageLabels付きPDFが主要文書になった場合、物理→論理ページ変換を純粋関数として追加する。
